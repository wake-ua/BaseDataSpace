...
# ============================================================================================================================== #
DEBUG = True
# Antes: ruta Windows fija. Ahora: DATA_DIR (default /app/datos para Docker)
DATA_PATH  = os.getenv('DATA_DIR', '/app/datos')
...
def get_model_max_tokens(model):
    ...
    try:
        ...
        return 512
# ============================================================================================================================== #
# Acceso a la bbdd (Qdrant): antes 127.0.0.1, ahora variable de entorno y sin TLS verify
API_URL     = os.getenv('QDRANT_URL', 'http://qdrant:6333')
API_KEY     = os.getenv('QDRANT_API_KEY', '')
# TLS fuera: no verificamos certificados en Docker
REQ_VERIFY  = False
...
def qdrant_upsert(name, ids, vectors, texts, extras=None):
    ...
    request = requests.put(
        f'{API_URL}/collections/{name}/points?wait=true',
...
def cargar_metadatos(dir_path):
    ...
    for fichero_json in tqdm(archivos, desc='Leyendo metadatos'):
        ...
        try:
            ...
            if DEBUG == True:
                print(f"[!] No se pudo leer '{fichero_json}': {e}", flush=True)
            continue
...
def media_embeddings_contenido_csv(resource, model, batch_size=256, max_rows=None):
    ...
    try:
        ...
        print(f"[contenido] No se pudo procesar {ruta_csv}: {e}", flush=True)
        return None, 0, '', False
    ...
    for fichero_json in tqdm(archivos, desc='Procesando metadatos/recursos (streaming)'):
        ...
        for res in resources:
            ...
            if res.get('path'):  # solo si hay archivo sample
                ...
                # ----- Contenidos -----
                media_contenido, n_filas_index, text_payload, was_truncated = media_embeddings_contenido_csv(
                    res, model, batch_size=LINES_BATCH_SIZE, max_rows=MAX_CONTENT_LINES
                )
...
def guardar_estadisticas_modelo(tiempo):
    ...
    try:
        response = requests.get(url, headers=headers, verify=REQ_VERIFY)
        text = response.text
        ...
        estadisticas = {
            ...
            # 'espacio_disco': get_cluster_gib()  # si tienes esa función
        }
...
if __name__ == '__main__':
    ...
    print("Inicializando modelo... (puede tardar varios segundos)", flush=True)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Dispositivo detectado: {device}. Preparando carga del modelo '{MODEL_NAME}'...", flush=True)

    # -------------------------------
    # Heartbeat: mensaje cada X segundos
    # -------------------------------
    heartbeat_active = True

    def heartbeat():
        while heartbeat_active:
            print("Cargando modelo... sigue en progreso", flush=True)
            time.sleep(5)
    ...
    try:
        ...
        heartbeat_active = False  # parar el heartbeat
